...
Part 1 stipulates that we must build the largest 2-digit numbers using digits from a string of 15-digits reading left-to-right. Once we have all of the largest 2-digit numbers, we should sum them. I suspect part 2 will have us build a longer number, so I'm keeping that in mind while I attempt to solve part 1.

Example: Using the digits: \`987654321111111\`, we can build \`98\` from the first two digits
Example: Using the digits \`818181911112111\`, we can build \`92\` using the center \`9\` and the latter \`2\`

Implementing this took me some time, but through discussions with friends, a key realization ended up leading me to my solution:

- Since the leftmost digit in any number is the most significant one, if you try to push each newly considered digit as far left as possible (we'll define "possible" in a second), you'll end with the largest number with the specified number of digits
